...
export function ArticleCard({ article }: ArticleCardProps) {
  return (
    <article className="surface-card group flex h-full flex-col overflow-hidden rounded-[28px]">
      <Link href={`/articles/${article.slug}`} className="relative block h-56 overflow-hidden bg-slate-200">
        {article.imageUrl ? (
          <Image
      ...
      <div className="flex flex-1 flex-col gap-4 p-6">
        ...
        <div className="space-y-3">
          <Link href={`/articles/${article.slug}`}>
            <h3 className="text-2xl font-black tracking-tight text-slate-950 transition group-hover:text-[var(--bmw-blue)]">
              {article.title}
            </h3>
          </Link>
